...
# Set a safer execution environment
set -euo pipefail
...
readonly BLUE='\e[34m'
readonly WHITE='\e[137m'       #'\e[37m' not bright, '\e[1;37m' bright
readonly BOLD='\e[1m'
readonly RESET='\e[0m'
...
tui_print_line() {
    ...
    local color="${1:-$YELLOW}"        # First argument: Color code (e.g., $RED, $BLUE)
    local char="${2:-=}"        # Second argument: Separator character (defaults to '=')
...
function tui_print_section() {
    ...
    local char="${3:-=}"        # 3. Second argument: Separator character (defaults to '=')

    tui_print_line "$color" "$char"
    tui_print_title "$section_text" "$color"
    tui_print_line "$color" "$char"
}
...
# Examples
# tui_print_title "$YELLOW" "TUI Functions" "#"
...
# Examples
# message="This is a crucial, but very long update that needs to stay within the terminal width, and therefor needs more and more text here."
# tui_print_line "$GREEN" "[O]" "$message"
# tui_print_line "$YELLOW" "[!]" "Configuration file not found."
...
# Examples
# tui_start_spinner "Downloading package updates (this will take 3 seconds)..."
# sleep 3
# action_result=$? # Capture the exit status of the command
# tui_stop_spinner "$action_result" "Package updates downloaded successfully."

# tui_start_spinner "Downloading package updates aaaaa bbbbb ccccc ddddd eeee ffffff ggggggg hhhhhhh  (this will take 3 seconds)..."
# sleep 3
# action_result=1 # Capture the exit status of the command
# tui_stop_spinner "$action_result" "Package updates downloaded failed."
