...
# Usage:
# loadkeys us
# curl https://raw.githubusercontent.com/echjansen/pure-arch/master/download.sh | bash
# vim pure-arch.conf
# ./pure-arch.sh
    ...
    if [ "$REBOOT" == "true" ]; then
        ...
        if [ "$REBOOT" == 'true' ]; then
            ...
            if [ "$ASCIINEMA" == "true" ]; then
                echo "Reboot aborted. You will must terminate asciinema recording and do a explicit reboot (exit, ./pure-arch-reboot.sh)."
                echo ""
            else
                echo "Reboot aborted. You will must do a explicit reboot (./pure-arch-reboot.sh)."
                echo ""
            fi
        ...
        if [ "$ASCIINEMA" == "true" ]; then
            echo "No reboot. You will must terminate asciinema recording and do a explicit reboot (exit, ./pure-arch-reboot.sh)."
            echo ""
        else
            echo "No reboot. You will must do a explicit reboot (./pure-arch-reboot.sh)."
            echo ""
        fi
